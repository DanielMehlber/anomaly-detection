...
def generate_test_video(output_path: str, duration_seconds: float = 120, fps: float = 30) -> None:
    ...
    for i in range(total_frames):
        ...
        visible_points = list(base_points)

        # Missing elements at t=88-92 — only two corner markers remain.
        if 88 <= t <= 92:
            visible_points = base_points[:2]

        _draw_control_screen(frame, visible_points, brightness=178 + drift)

        # Flicker at t=75-78 — strong alternating brightness every frame.
        if 75 <= t <= 78 and i % 2 == 0:
            frame = np.clip(frame.astype(np.int16) + 55, 0, 255).astype(np.uint8)

        # Black screen at t=95-97.
        if 95 <= t <= 97:
            frame[:] = 5

        # Spatial distortion at t=105-112, then scene returns to normal.
        if 105 <= t <= 112:
            frame = np.zeros((height, width, 3), dtype=np.uint8)
            offset = int((t - 105) * 12)
            shifted = [
                (min(x + offset, width - 80), min(y + offset // 2, height - 80))
